...
import { PRODUCT_BY_ID, riskName, classSiblings } from "./data/wealthProducts";
...
export default function WealthDetailPage() {
  ...
  return <HubShell>
    ...
    <nav aria-label="상품 상세 구역" style={{gridTemplateColumns:`repeat(${bank.data ? 4 : 3},minmax(0,1fr))`}} className="sticky top-16 z-10 my-5 grid rounded-xl border border-slate-200 bg-white/95 p-1 shadow-sm backdrop-blur">{[["summary","상담 요점"],["cost","비용·환매"],["performance","운용 현황"],...(bank.data ? [["holdings","보유종목"]] : [])].map(([key,label])=><a key={key} href={`#product-${key}`} className="flex min-h-11 items-center justify-center rounded-lg px-1 py-2 text-center text-xs font-semibold text-slate-700 hover:bg-sky-50 hover:text-sky-800 sm:text-sm">{label}</a>)}</nav>
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_240px]">
      ...
      <aside aria-label="상품 상담 도구" className="rounded-xl border border-slate-200 bg-white p-4 lg:sticky lg:top-36">
        ...
        <FundDocumentButtons product={product} className="mb-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-sky-700 px-3 py-3 text-sm font-bold text-white hover:bg-sky-800"/>
