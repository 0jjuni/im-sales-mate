...
- **git이 유일한 핸드오프 채널이다.** 두 도구는 서로 실시간 메시지를 못 주고받는다.
- 그래서 각 작업은 반드시:
  1. **시작 전 `git pull`** — 상대가 올린 최신 상태 위에서 작업.
  2. 작업 후 **`git commit && git push`** — 작게, 자주. 완결된 단위로 커밋해 상대가 깨끗한 트리를 받게 한다.
- **작업 트리는 공유된다** (같은 폴더). 시작할 때 `git status`에 내가 만들지 않은 미커밋 변경이 보이면, 그건 상대가 작업 중인 것이다 → **내 것으로 커밋하지 말고**, 그 파일은 건드리지 않거나 사용자에게 상황을 알린다.
- 겹치는 파일을 부득이 수정했다면 커밋 메시지에 무엇을·왜 바꿨는지 남겨 상대가 추적할 수 있게 한다.
