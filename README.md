...
| 영역 | 내용 |
|---|---|
| **허브 대시보드** (`/`) | 마켓 보드 · AI 모닝 브리핑(아침 필수 뉴스) · 내 도구(개인화 퀵 액세스) · 상품 모듈 · PB 지식 라이브러리 |
...
`src/hub/sections.js`에 섹션 등록 → `HubHome.jsx`의 `renderSection()`에 렌더러 추가. 순서·숨김·네비게이션 노출은 자동으로 개인화에 편입됩니다(기존 사용자 저장분에는 새 섹션이 맨 뒤에 자동 병합).
